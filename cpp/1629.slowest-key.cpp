...
public:
    char slowestKey(vector<int>& releaseTimes, string keysPressed) {
        char longest_key_pressed = keysPressed.at(0);
        int longest_duration = releaseTimes.at(0);
        for (int i = 1; i < releaseTimes.size(); i++) {
            int current_duration = releaseTimes.at(i) - releaseTimes.at(i - 1);
            if (current_duration > longest_duration) {
                longest_duration = current_duration;
                longest_key_pressed = keysPressed.at(i);
            } else if (current_duration == longest_duration) {
                if (keysPressed.at(i) > longest_key_pressed) {
                    longest_key_pressed = keysPressed.at(i);
                }
            }
        }
