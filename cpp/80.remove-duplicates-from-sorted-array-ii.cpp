...
public:
    int removeDuplicates(vector<int>& nums) {
        int next_index_to_write = 1;
        int current_num = nums.at(0);
        ...
        for (int i = 1; i < nums.size(); i++) {
            ...
            if (current_repetitions <= 2) {
                nums[next_index_to_write] = nums[i];  // move it to the right place
                next_index_to_write += 1;
            }
        }
        return next_index_to_write;
    }
};
