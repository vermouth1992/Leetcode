...
class Solution:
    def leastBricks(self, wall: List[List[int]]) -> int:
        # for each row, compute the index where there is a hole. Then cut the index with the maximum holes
        index_to_holes = dict()
        for row in wall:
            column = 0
            for brick in row[:-1]:
                column += brick
                if column not in index_to_holes:
                    index_to_holes[column] = 0
                index_to_holes[column] += 1
        
        # there is no holes
        if len(index_to_holes) == 0:
            return len(wall)

        return len(wall) - max(index_to_holes.values())
