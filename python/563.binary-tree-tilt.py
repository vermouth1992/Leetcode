...
class Solution:
    ...
    def findTiltHelper(self, root: TreeNode):
        # return sum of the root and its tilt
        if root is None:
            return 0, 0
        leftSum, leftTilt = self.findTiltHelper(root.left)
        rightSum, rightTilt = self.findTiltHelper(root.right)
        currentTile = abs(leftSum - rightSum)
        return root.val + leftSum + rightSum, leftTilt + rightTilt + currentTile
