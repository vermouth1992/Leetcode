...
class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        ...
        tasks_freq = dict()
        for task in tasks:
            if task not in tasks_freq:
                tasks_freq[task] = 0
            tasks_freq[task] += 1
        # create a (task, freq) pair list
        tasks_freq = list(tasks_freq.items())
        tasks_freq = sorted(tasks_freq, key=lambda x: x[1], reverse=True)
        output = []
        for task, freq in tasks_freq:
