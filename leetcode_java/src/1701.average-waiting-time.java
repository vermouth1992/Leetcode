...
class Solution1701 {
    public double averageWaitingTime(int[][] customers) {
        ...
        for (int i = 0; i < customers.length; i++) {
            if (customers[i][0] < nextAvailableTime) {
                totalWaitTime = totalWaitTime + nextAvailableTime - customers[i][0] + customers[i][1];
                nextAvailableTime = nextAvailableTime + customers[i][1];
            } else {
                totalWaitTime = totalWaitTime + customers[i][1];
    ...
    public static void main(String[] args) {
        Solution1701 s = new Solution1701();
        System.out.println(s.averageWaitingTime(new int[][]{{5,2},{5,4},{10,3},{20,1}}));
    }
}
